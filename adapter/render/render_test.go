...
func TestMakeSplitBar(t *testing.T) {
	...
	// ▇ = both (usage + time overlap, red peeks through bottom)
	// █ = usage only (no time underneath)
	// ▁ = time only (thin red line)
	// ░ = empty
	tests := []struct {
		name      string
		usage     int
		...
		width     int
		wantBoth  int // ▇ count
		wantUsage int // █ count (usage-only)
		wantTime  int // ▁ count (time-only)
		wantNone  int // ░ count
	}{
		{"equal", 50, 50, 8, 4, 0, 0, 4},
		{"usage ahead", 60, 30, 10, 3, 3, 0, 4},
		{"time ahead", 20, 60, 10, 2, 0, 4, 4},
		{"both empty", 0, 0, 8, 0, 0, 0, 8},
		{"both full", 100, 100, 8, 8, 0, 0, 0},
		{"clamped", 150, -10, 8, 0, 8, 0, 0},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := r.MakeSplitBar(tt.usage, tt.time, tt.width)
			both := strings.Count(got, "▇")
			usage := strings.Count(got, "█")
			timeLine := strings.Count(got, "▁")
			none := strings.Count(got, "░")
			if both != tt.wantBoth {
				t.Errorf("▇ count: got %d, want %d", both, tt.wantBoth)
			}
			if usage != tt.wantUsage {
				t.Errorf("█ count: got %d, want %d", usage, tt.wantUsage)
			}
			if timeLine != tt.wantTime {
				t.Errorf("▁ count: got %d, want %d", timeLine, tt.wantTime)
			}
			if none != tt.wantNone {
